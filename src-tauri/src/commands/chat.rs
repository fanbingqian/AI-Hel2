...
use crate::services::agents::agent_interface::ChatMessage;
...
/// Read the default agent's main model from agents.json.
                ...
                for a in agents {
                    if a["id"].as_str() == Some(default_id) && a["enabled"].as_bool().unwrap_or(true) {
                        let ms: Vec<_> = a["config"]["models"].as_array()
                            ...
                            .unwrap_or_default();
                        return ms.first().copied().unwrap_or("deepseek-v4-flash").to_string();
                    }
                }
            }
        }
...
pub async fn chat_completions(
    ...
    let model = model.filter(|m| !m.is_empty()).unwrap_or_else(resolve_default_model);
